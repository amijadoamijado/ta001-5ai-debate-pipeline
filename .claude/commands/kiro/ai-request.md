---
description: Generate AI implementation/analysis request document from Kiro task
...
Generate structured request documents for AI agents (Gemini/Claude/Codex) from Kiro specifications.

**NOTE**: This command uses the unified AI coordination workflow. All documents are saved to `.kiro/ai-coordination/workflow/`.

## Command Usage

```bash
# Basic usage with task ID
/kiro:ai-request <feature-name> <task-id> --agent=<gemini|claude|codex>

# Task range
/kiro:ai-request <feature-name> <task-range> --agent=<ai-name>

# Phase-based generation
/kiro:ai-request <feature-name> --phase=<N> --agent=<ai-name>
```
...
/kiro:ai-request document-conversion-service 1.1 --agent=gemini

# Claude analysis request
/kiro:ai-request document-conversion-service 2.1 --agent=claude

# Codex refactoring request
/kiro:ai-request document-conversion-service 3.1 --agent=codex

# Phase 1 to Gemini
/kiro:ai-request document-conversion-service --phase=1 --agent=gemini

# Task range
/kiro:ai-request document-conversion-service 1.1-1.2 --agent=gemini
```
...
if [ -z "$FEATURE_NAME" ]; then
  ...
  echo "Usage: /kiro:ai-request <feature-name> <task-id> --agent=<ai-name>"
  exit 1
fi
...
if [ -n "$PHASE" ]; then
  ...
  echo "Phase: $PHASE"
  TASK_FILE=".kiro/specs/$FEATURE_NAME/tasks/phase${PHASE}.md"
...
{{#read}}
.kiro/specs/$ARGUMENTS[0]/requirements.md
{{/read}}

{{#read}}
.kiro/specs/$ARGUMENTS[0]/design.md
{{/read}}

{{#read}}
.kiro/specs/$ARGUMENTS[0]/tasks/phase*.md
{{/read}}
...
5. **Save to** `.kiro/ai-coordination/workflow/spec/{feature-name}/IMPLEMENT_REQUEST_{task-id}.md`
...
if [ -n "$PHASE" ]; then
  ...
  OUTPUT_FILE=".kiro/ai-coordination/workflow/spec/${PROJECT_ID}/IMPLEMENT_REQUEST_${TASK_NUM}.md"
else
...
mkdir -p ".kiro/ai-coordination/workflow/spec/${PROJECT_ID}/"
...
- [ ] Report file: .kiro/ai-coordination/workflow/review/${PROJECT_ID}/REVIEW_IMPL_${TASK_NUM}.md

## Next Steps
1. Complete implementation/analysis
2. Register report: /kiro:ai-report $FEATURE_NAME $TASK_OR_PHASE --agent=$AGENT
...
echo "     /kiro:ai-report $FEATURE_NAME $TASK_OR_PHASE --agent=$AGENT"
{{/exec}}
...
**Generated:**
- Request document: `.kiro/ai-coordination/workflow/spec/{project-id}/IMPLEMENT_REQUEST_{NNN}.md`
- Updated: `.kiro/specs/[feature]/spec.json`

**Next Actions:**
1. Review and customize the request document
2. Send to the specified AI agent
3. After completion, use `/kiro:ai-report` to register the report
