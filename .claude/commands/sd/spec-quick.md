...
- **Mission**: Execute all spec phases (init 竊・requirements 竊・design 竊・tasks) in a single command
- **Success Criteria**:
...
<instructions>
## 笞・・CRITICAL: Automatic Mode Execution Rules
...
- Phase 1 complete = 1/4 tasks done 竊・Continue to Phase 2
- Phase 2 complete = 2/4 tasks done 竊・Continue to Phase 3
- Phase 3 complete = 3/4 tasks done 竊・Continue to Phase 4
- Phase 4 complete = 4/4 tasks done 竊・Output summary and exit
...
Example:
```
"User profile with avatar upload --auto" 竊・mode=automatic, description="User profile with avatar upload"
"User profile feature" 竊・mode=interactive, description="User profile feature"
```
...
1. **Generate Feature Name**:
   ...
   - Example: "User profile with avatar upload" 竊・"user-profile-avatar-upload"
   - Keep name concise (2-4 words ideally)

2. **Check Uniqueness**:
   - Use Glob to check `.sd/specs/*/`
   - If feature name exists, append `-2`, `-3`, etc.

3. **Create Directory**:
   - Use Bash: `mkdir -p .sd/specs/{feature-name}`

4. **Initialize Files from Templates**:

   a. Read templates:
   ```
   - .sd/settings/templates/specs/init.json
   - .sd/settings/templates/specs/requirements-init.md
   ```

   b. Replace placeholders:
   ```
   {{FEATURE_NAME}} 竊・feature-name
   {{TIMESTAMP}} 竊・current ISO 8601 timestamp (use `date -u +"%Y-%m-%dT%H:%M:%SZ"`)
   {{PROJECT_DESCRIPTION}} 竊・description
   ```

   c. Write files using Write tool:
   ```
   - .sd/specs/{feature-name}/spec.json
   - .sd/specs/{feature-name}/requirements.md
   ```
...
6. **Output Progress**:
   ```
   笨・Spec initialized at .sd/specs/{feature-name}/
   ```
...
```
/sd:spec-requirements {feature-name}
```

Wait for completion. SubAgent will return with "谺｡縺ｮ繧ｹ繝・ャ繝・ message.

**IMPORTANT**: In Automatic Mode, IGNORE the "谺｡縺ｮ繧ｹ繝・ャ繝・ message. It is for standalone usage.

**Update TodoWrite**: Mark task 2 as `completed`, task 3 as `in_progress`.

**Output Progress**:
```
笨・Requirements generated 竊・Continuing to design...
```
...
**IMPORTANT**: In Automatic Mode, IGNORE the "谺｡縺ｮ繧ｹ繝・ャ繝・ message.

**Update TodoWrite**: Mark task 3 as `completed`, task 4 as `in_progress`.

**Output Progress**:
```
笨・Design generated 竊・Continuing to tasks...
```
...
- Do NOT be influenced by "谺｡縺ｮ繧ｹ繝・ャ繝・ messages from Phases 2-4
...
### Phase 1 Tools
- **Glob**: Check `.sd/specs/*/` for existing feature names
- **Bash**: Create directory with `mkdir -p`, generate timestamp with `date -u`
- **Read**: Fetch templates from `.sd/settings/templates/specs/`
- **Write**: Create `spec.json` and `requirements.md` in spec directory

### Phase 2-4 Tools
- **SlashCommand**: Execute `/sd:spec-requirements`, `/sd:spec-design`, `/sd:spec-tasks`
...
**Interactive Mode**:
```
噫 Quick Spec Generation (Interactive Mode)

You will be prompted at each phase.
笞・・Skips gap analysis and design validation.
```

**Automatic Mode**:
```
噫 Quick Spec Generation (Automatic Mode)

All phases execute automatically without prompts.
笞・・Skips all validations and reviews.
```
...
```
笨・Spec initialized at .sd/specs/{feature}/
笨・Requirements generated 竊・Continuing to design...
笨・Design generated 竊・Continuing to tasks...
```
...
```
笨・Quick Spec Generation Complete!

## Generated Files:
- .sd/specs/{feature}/spec.json
- .sd/specs/{feature}/requirements.md ({X} requirements)
- .sd/specs/{feature}/design.md ({Y} components, {Z} endpoints)
- .sd/specs/{feature}/tasks.md ({N} tasks)

笞・・Quick generation skipped:
- `/sd:validate-gap` - Gap analysis (integration check)
- `/sd:validate-design` - Design review (architecture validation)

## Next Steps:
1. Review generated specs (especially design.md)
2. Optional validation:
   - `/sd:validate-gap {feature}` - Check integration with existing codebase
   - `/sd:validate-design {feature}` - Verify architecture quality
3. Start implementation: `/sd:spec-impl {feature}`

## Note:
For complex features (integrations, security, APIs), use standard workflow:
/sd:spec-init 竊・/sd:spec-requirements 竊・/sd:validate-gap
竊・/sd:spec-design 竊・/sd:validate-design 竊・/sd:spec-tasks
```
...
- Example: `"Multi word description --auto"` 竊・extract both parts correctly
...
**Template Missing**:
- Check `.sd/settings/templates/specs/` exists
...
- Suggest: "Continue manually from `/sd:spec-{next-phase} {feature}`"
