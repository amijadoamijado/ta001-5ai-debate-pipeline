...
- Parse conversation history for `/sd:spec-impl <feature> [tasks]` commands
...
- If no history found, scan `.sd/specs/` for features with completed tasks `[x]`

**If feature provided** (`$1` present, `$2` empty):
- Use specified feature
- Detect all completed tasks `[x]` in `.sd/specs/$1/tasks.md`
...
- Read `.sd/specs/<feature>/spec.json` for metadata
- Read `.sd/specs/<feature>/requirements.md` for requirements
- Read `.sd/specs/<feature>/design.md` for design structure
- Read `.sd/specs/<feature>/tasks.md` for task list
- **Load ALL steering context**: Read entire `.sd/steering/` directory including:
...
## Tool Guidance
- **Conversation parsing**: Extract `/sd:spec-impl` patterns from history
...
- Flag critical issues with 笞・・or 閥
- Keep summary concise (under 400 words)

## Safety & Fallback

### Error Scenarios
- **No Implementation Found**: If no `/sd:spec-impl` in history and no `[x]` tasks, report "No implementations detected"
...
- Re-run `/sd:spec-impl <feature> [tasks]` for fixes
- Re-validate with `/sd:validate-impl [feature] [tasks]`
