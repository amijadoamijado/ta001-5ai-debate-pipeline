...
## Execution Steps
1. **Check Uniqueness**: Verify `.sd/specs/` for naming conflicts (append number suffix if needed)
2. **Create Directory**: `.sd/specs/[feature-name]/`
3. **Initialize Files Using Templates**:
   - Read `.sd/settings/templates/specs/init.json`
   - Read `.sd/settings/templates/specs/requirements-init.md`
   - Replace placeholders:
     - `{{FEATURE_NAME}}` 竊・generated feature name
     - `{{TIMESTAMP}}` 竊・current ISO 8601 timestamp
     - `{{PROJECT_DESCRIPTION}}` 竊・$ARGUMENTS
...
4. **Next Step**: Command block showing `/sd:spec-requirements <feature-name>`
...
- **Template Missing**: If template files don't exist in `.sd/settings/templates/specs/`, report error with specific missing file path and suggest checking repository setup
