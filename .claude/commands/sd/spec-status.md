...
- Read `.sd/specs/$1/spec.json` for metadata and phase status
- Read existing files: `requirements.md`, `design.md`, `tasks.md` (if they exist)
- Check `.sd/specs/$1/` directory for available files
...
**Format**: Clear, scannable format with emojis (笨・竢ｳ/笶・ for status
...
**Spec Not Found**:
- **Message**: "No spec found for `$1`. Check available specs in `.sd/specs/`"
...
- Shows all specs in `.sd/specs/` with their status
