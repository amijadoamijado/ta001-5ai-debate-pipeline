...
- Design document must exist: `.sd/specs/$1/design.md`
- If not exist, stop with message: "Run `/sd:spec-design $1` first to generate design document"

### Review Context
- Spec metadata: @.sd/specs/$1/spec.json
- Requirements document: @.sd/specs/$1/requirements.md
- Design document: @.sd/specs/$1/design.md
- Core steering documents:
  - Architecture: @.sd/steering/structure.md
  - Technology: @.sd/steering/tech.md
  - Product context: @.sd/steering/product.md
- Custom steering: All additional `.md` files in `.sd/steering/` directory
...
**Issue Format**:
```
閥 **Critical Issue [1-3]**: [Brief title]
...
Generate review in the language specified in spec.json (check `.sd/specs/$1/spec.json` for "language" field):
...
Run `/sd:spec-tasks $1` to generate implementation tasks

**Auto-approve and proceed:**
Run `/sd:spec-tasks $1 -y` to auto-approve requirements and design, then generate tasks directly