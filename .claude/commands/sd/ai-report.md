...
**NOTE**: This command uses the unified AI coordination workflow. All reports are saved to `.sd/ai-coordination/workflow/review/`.

## Command Usage

```bash
# Basic usage
/sd:ai-report <feature-name> <task-id> --agent=<gemini|claude|codex>

# With file path
/sd:ai-report <feature-name> <task-id> --agent=<ai-name> --file=<report-path>
```
...
/sd:ai-report document-conversion-service 1.1 --agent=gemini

# Register Claude analysis report with file
/sd:ai-report document-conversion-service 2.1 --agent=claude --file=./analysis.md

# Register Codex refactoring report
/sd:ai-report document-conversion-service 3.1 --agent=codex
```
...
if [ -z "$FEATURE_NAME" ]; then
  ...
  echo "Usage: /sd:ai-report <feature-name> <task-id> --agent=<ai-name>"
  exit 1
fi
...
REQUEST_FILE=".sd/ai-coordination/workflow/spec/${PROJECT_ID}/IMPLEMENT_REQUEST_${TASK_NUM}.md"
...
OUTPUT_FILE=".sd/ai-coordination/workflow/review/${PROJECT_ID}/REVIEW_${REPORT_TYPE}_${TASK_NUM}.md"

# Ensure directory exists
mkdir -p ".sd/ai-coordination/workflow/review/${PROJECT_ID}/"
...
## References
- Request: .sd/ai-coordination/workflow/spec/${PROJECT_ID}/IMPLEMENT_REQUEST_${TASK_NUM}.md
...
REPORT_PATH=".sd/ai-coordination/workflow/review/${PROJECT_ID}/REVIEW_${REPORT_TYPE}_${TASK_NUM}.md"
...
echo "  /sd:spec-status $FEATURE_NAME"
{{/exec}}

## Summary

**Registered:**
- Report file: `.sd/ai-coordination/workflow/review/{project-id}/REVIEW_{TYPE}_{NNN}.md`
...
3. Check overall progress: `/sd:spec-status [feature-name]`
