...
### Steering Context
- Architecture context: @.sd/steering/structure.md
- Technical constraints: @.sd/steering/tech.md
- Product context: @.sd/steering/product.md
- Custom steering: Load all "Always" mode custom steering files from .sd/steering/

### Existing Spec Context
- Current spec directory: !`bash -c 'ls -la .sd/specs/$1/ 2>/dev/null || echo "No spec directory found"'`
- Requirements document: @.sd/specs/$1/requirements.md
- Spec metadata: @.sd/specs/$1/spec.json

## Task: Implementation Gap Analysis

### Prerequisites
- Requirements document must exist: `.sd/specs/$1/requirements.md`
- If not exist, stop with message: "Run `/sd:spec-requirements $1` first to generate requirements"
...
Generate analysis in the language specified in spec.json (check `.sd/specs/$1/spec.json` for "language" field):
...
Run `/sd:spec-design $1` to create technical design document

**Auto-approve and proceed:**  
Run `/sd:spec-design $1 -y` to auto-approve requirements and generate design directly